...
def test_integration_process_email_publishes_to_rabbitmq(email_processor_instance, mocker):
    ...
    processor._save_attachments.assert_called_once_with("integration_test_id_456")
    processor._is_spam.assert_called_once_with("integration_sender@example.com")
    processor._move_to_junk.assert_not_called() # Should not be called if not spam
    ...
    published_message = json.loads(call_kwargs['body'])
